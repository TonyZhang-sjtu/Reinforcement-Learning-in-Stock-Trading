...
def set_seed(seed=42):
    ...
    # NumPy
    np.random.seed(seed)
    gym.envs.registry.all()  # Initializes all environments
    # Ensure your environment `env` is defined properly and call:
...
def train_A2C(env_train, model_name, timesteps=50000):
    ...
    model = A2C('MlpPolicy', env_train, verbose=0, seed=42) # 创建一个 A2C 模型
...
def train_DDPG(env_train, model_name, timesteps=50000):
    ...
    start = time.time()  # 记录开始时间
    model = DDPG('MlpPolicy', env_train, seed=42, n_cpu_tf_sess=1) # 创建一个 DDPG 模型
    model.learn(total_timesteps=timesteps) # 在训练环境上训练模型
    end = time.time() # 记录结束时间
...
def train_PPO(env_train, model_name, timesteps=50000):
    """PPO model"""
    start = time.time() # 记录开始时间
    model = PPO2('MlpPolicy', env_train, seed=42, n_cpu_tf_sess=1) # 创建一个 PPO 模型
    model.learn(total_timesteps=timesteps) # 在训练环境上训练模型
    end = time.time() # 记录结束时间
...
def train_SAC(env_train, model_name, timesteps=50000):
    """SAC model"""
    start = time.time() # 记录开始时间
    model = SAC('MlpPolicy', env_train, verbose=0, seed=42, n_cpu_tf_sess=1) # 创建一个 SAC 模型
    model.learn(total_timesteps=timesteps) # 在训练环境上训练模型
    end = time.time() # 记录结束时间
...
def train_TD3(env_train, model_name, timesteps=50000):
    """TD3 model"""
    start = time.time() # 记录开始时间
    model = TD3('MlpPolicy', env_train, verbose=0, seed=42, n_cpu_tf_sess=1) # 创建一个 SAC 模型
    model.learn(total_timesteps=timesteps) # 在训练环境上训练模型
    end = time.time() # 记录结束时间
...
def run_ensemble_strategy(df, unique_trade_date, rebalance_window, validation_window) -> None:
    ...
    for i in range(rebalance_window + validation_window, len(unique_trade_date), rebalance_window):
        ...
        # # 训练 PPO 模型
        # model_ppo = train_PPO(env_train, model_name="PPO_100k_dow_{}".format(i), timesteps=10000) # 训练 PPO 模型
        ...
        print("======DDPG Training========") # DDPG最好的timestep选项是5000
        ...
        # print("======SAC Training========")
        # model_sac = train_SAC(env_train, model_name="SAC_10k_dow_{}".format(i), timesteps=10000) # 训练 SAC 模型
        ...
        print("======TD3 Training========") # TD3最好的情况是5000
        ...
        # sac_sharpe_list.append(sharpe_sac) # 记录 SAC 模型的夏普比率
        td3_sharpe_list.append(sharpe_td3) # 记录 TD3 模型的夏普比率

        # model_ensemble = model_ddpg
        # model_use.append('DDPG')

        if (sharpe_ddpg >= sharpe_td3): # 如果 DDPG 的夏普比率最高 
            model_ensemble = model_ddpg
            model_use.append('DDPG')
        # elif (sharpe_a2c >= sharpe_td3):
        ...
        # if (sharpe_ddpg >= sharpe_ppo) and (sharpe_ddpg >= sharpe_td3) and (sharpe_ddpg >= sharpe_a2c): # 如果 PPO 的夏普比率最高 
        #     model_ensemble = model_ddpg
        #     model_use.append('DDPG')
        # elif (sharpe_ppo >= sharpe_td3) and (sharpe_ppo >= sharpe_a2c):
        #     model_ensemble = model_ppo
        #     model_use.append('PPO')
        # elif (sharpe_td3 >= sharpe_a2c):
        #     model_ensemble = model_td3
        #     model_use.append('TD3')
        # else:
        #     model_ensemble = model_a2c
